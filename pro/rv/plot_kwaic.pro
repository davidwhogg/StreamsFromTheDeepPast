...
;      plot_kwAIC
;   PURPOSE:
;      plot the AIC surface in (k,w) FOR THE ALMS sample,
;      new reduction!
;   CALLING SEQUENCE:
;   INPUT:
...
;-
PRO PLOT_KWAIC, savefilename=savefilename, plotfilename=plotfilename, $
                horizontal=horizontal, basedir=basedir, ks_low=ks_low, $
                ks_high=ks_high, ks=ks, ws=ws
...
;;Read the data
cmd= '../make_aic_file.sh '+strtrim(string(ks[0]),2)+' '+strtrim(string(ks[nks-1]),2)+' '+basedir
spawn, cmd
readcol, 'aic.dat', aic, format='D'
cmd= 'rm -f aic.dat'
spawn, cmd
...
FOR kk=0L, nks-1 DO FOR ww= 0L, nws-1 DO BEGIN
    loglike[kk,ww]= aic[ww+kk*nws]
ENDFOR


;;Print the max
min_indx= where(aic EQ min(aic))
splog, "Min= "+strtrim(string(aic(min_indx[0])),2)+" @: K= "+strtrim(string(ks[min_indx[0]/nws]),2)+", w= "+strtrim(string(ws[min_indx[0] MOD nws],format='(F4.2)'),2)
...
IF keyword_set(horizontal) THEN BEGIN
    ...
    axis, xaxis=1, charsize=.45*charsize, xtickformat='(E10.3)', $
      xticks=nticks, charthick=.6*charthick
    djs_plot, [0.],[0.], /NOERASE, title='AIC', $
