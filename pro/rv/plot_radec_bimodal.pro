...
;      sphere (assumed to be the most bimodal), and plot the predicted
;      distribution + observed distribution from GCS for this
;      direction
;   CALLING SEQUENCE:
;   INPUT:
...
;      filename - filename to save the bimodalities
...
;      patches  - number of patches to calculate the bimodality in
;   KEYWORDS:
;      QA      - print QA info
;      highentropy - (don't ask) plot high values of the bimodality
;                    (that is, not very bimodal)
;   OUTPUT:
;   REVISION HISTORY:
;      2009-01-20 - Written Bovy (NYU)
;-
PRO PLOT_RADEC_BIMODAL, K=K, w=w, sample=sample, filename=filename, $
...
IF ~keyword_set(basedir) THEN basedir='~/streams/pro/rv/ALMSv2_reconverge/'
...
ENDIF ELSE BEGIN
    ...
    bimodal= dblarr(nras,ndecs)
    bimodaloned= dblarr(nras*ndecs)
    ...
    FOR ii= 0L, nras-1 DO FOR jj= 0L, ndecs-1 DO BEGIN
        ...
        ENDFOR
        ;;Actual calculation of the bimodality
        sample_gaussians, nsamples=10000, mean=rmean_marg, $
          covar=rcovar_marg_noerr, amp=amp, sample=sample, seed=seed
        
        bimodal[ii,jj]= bimodality_scalar(sample)
        bimodaloned[ii*ndecs+jj]= bimodal[ii,jj]

    ENDFOR
    IF keyword_set(filename) THEN BEGIN
        IF keyword_set(QA) THEN splog, "Saving entropies in "+filename
        save, filename=filename, ras, decs, nras, ndecs, ra_bin, $
          dec_bin, bimodal, bimodaloned
    ENDIF
ENDELSE
...
sort_indx= sort(bimodaloned)
sorted_bimodaloned= bimodaloned[sort_indx]
plotted_ras= dblarr(npatch)-1.
plotted_decs= dblarr(npatch)-91.
;max_info_indx= where(bimodaloned EQ min(bimodaloned))
...
FOR ii= 0L, npatch-1 DO BEGIN
    ...
    plot_this_entropy= sorted_bimodaloned[possible_stars]
    IF keyword_set(QA) THEN splog, "Plotting bimodal "+strtrim(string(plot_this_entropy,format='(F5.3)'),2)
    ra= ras[where((bimodaloned EQ plot_this_entropy))/ndecs]
    dec= decs[where((bimodaloned EQ plot_this_entropy)) MOD ndecs]
    plotted_ras[ii]= ra
    plotted_decs[ii]= dec
    ...
    n_in_patch= n_elements(indx_patch)
...
;;Plot
    ...
    IF (ii MOD mplot[1]) EQ 0 THEN BEGIN
        IF ii/mplot[1] EQ mplot[2]-1 THEN BEGIN
            ...
            hogg_plothist, gcs[indx_patch].vr, npix=npix, /overplot, $
              weight=weight
        ENDIF ELSE BEGIN
            djs_plot, rs, ds_marg, xrange=rrange, yrange=yrange, $
