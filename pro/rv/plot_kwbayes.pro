...
;      plot_kwBayes
;   PURPOSE:
;      plot the Bayesian evidence surface in (k,w) FOR THE ALMS sample,
;      new reduction!
;   CALLING SEQUENCE:
;   INPUT:
...
;-
PRO PLOT_KWBAYES, filename=filename, plotfilename=plotfilename, $
...
bayestwo= dblarr(nks,nws)
bayesone= dblarr(nks*nws)
...
ENDIF ELSE BEGIN
    ...
    ;;Now run through the different models to find the evidence for each
    FOR kk=0L, nks-1 DO FOR ww=0L, nws-1 DO Begin
        ...
        ;;Calculate the posterior probabilities qij
        assign_clump_members, logpost, ydata, ycovar, projection, $
          mean, covar, amp
        ;;Calculate evidence
        ML= 0D0
        ...
        FOR kk2= 0L, ks[kk]-1 DO BEGIN
            ...
            FOR dd= 0L, d-1 DO ML-= 2D0*alog(sigmas[dd])
            ML+= 2D0*d*alog(sqrt(2D0)*nhip*amp[kk2])
        ENDFOR
        ;;Nasty term
        IF ks[kk] NE 1 THEN BEGIN
            FOR kk2=0L, ks[kk]-2 DO BEGIN
                logarg= 0D0
                FOR ii=0L, nhip-1 DO $
                  logarg+= (exp(logpost[kk2,ii]-alog(amp[kk2]))-$
                            exp(logpost[ks[kk]-1,ii]-alog(amp[ks[kk]-1])))^2
                ML-= 0.5D0*alog(logarg > minamp)
            ENDFOR
        ENDIF
        ;;Add likelihood
        ML+= avgloglikedata*nhip
        bayestwo[kk,ww]= ML
        bayesone[ww+kk*nws]= ML
    ENDFOR
    IF keyword_set(filename) THEN BEGIN
        save, bayestwo, ks,ws,bayesone, filename=filename
        splog, "Saved Bayesian evidence surface in "+filename
    ENDIF
ENDELSE
...
ml= bayesone
;;Print the max
max_indx= where(ml EQ max(ml))
splog, "Max= "+strtrim(string(ml(max_indx[0])),2)+$
  " @: K= "+strtrim(string(ks[max_indx[0]/nws]),2)+$
  ", w= "+strtrim(string(ws[max_indx[0] MOD nws],format='(F4.2)'),2)
...
IF keyword_set(horizontal) THEN BEGIN
    ...
    axis, xaxis=1, charsize=.45*charsize, xtickformat='(E10.3)', $
      xticks=nticks, charthick=.6*charthick
    djs_plot, [0.],[0.], /NOERASE, title='Bayesian evidence', $
