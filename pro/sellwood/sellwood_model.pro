...
PRO SELLWOOD_MODEL, sample=sample, disk=disk, vc=vc, ro= ro, $
                    ...
                    output=output, justhyades=justhyades
...
IF sample EQ 'GCS' THEN BEGIN
    gcssavefilename= '../groups/phot_gcs.sav'
...
IF keyword_set(justhyades) THEN BEGIN
    ...
    xwidth=.55
    ywidth=.4
    allposition= [xmargin,ymargin+ywidth,xmargin+xwidth,ywidth*2+ymargin]
    hyadesposition= [xmargin,ymargin,xmargin+xwidth,ywidth+ymargin]
    ;;All first
    djs_plot, EJ[*,1], EJ[*,0],position=allposition,$
      psym=3, yrange=[0.,0.05], xrange=[0.7,1.3],charsize=charsize, xtickname=strarr(30)+' ', $
      ytitle='(E-E_c) V_c^{-2}'
    legend, ['All stars'], box=0.,charsize=legendcharsize,pos=[1.15,.0097]
    djs_plot, [0,1],[0,1],/nodata, $
      position=[xmargin,ymargin+ywidth,xmargin+xwidth,ywidth*2+ymargin], $
    ...
    djs_plot, EJ[hyades_members,1], EJ[hyades_members,0],$
      ...
      psym=3, yrange=[0.,0.05], xrange=[0.7,1.3],charsize=charsize, $
      ytitle='(E-E_c) V_c^{-2}', xtitle='L/L_c(R_0)'
    legend, ['Hyades'], box=0.,charsize=legendcharsize,pos=[1.15,.0097]
    k_end_print
ENDIF ELSE IF output EQ 1 THEN BEGIN
    ;;Calculate E and J for all of the stars
    EJ= dblarr(ngcs,2)
    FOR ii=0L, ngcs-1 DO BEGIN
        l= gcs[ii].l/180.*!DPI
...
ENDIF ELSE IF output EQ 2 THEN BEGIN
    ...
    IF keyword_set(colour) THEN BEGIN
        djs_oplot, EJ[ngc1901_members,1], EJ[ngc1901_members,0],$
          color='red', psym=3
        legend, ['NGC 1901'], box=0.,charsize=charsize,textcolors=djs_icolor('red'),pos=[.7,.02]
        djs_oplot, EJ[sirius_members,1], EJ[sirius_members,0],$
          color='blue',psym=3
        legend, ['Sirius'], box=0.,charsize=charsize,textcolors=djs_icolor('blue'),pos=[.7,.017]
        djs_oplot, EJ[pleiades_members,1], EJ[pleiades_members,0],$
          color='green',psym=3
        legend, ['Pleiades'], box=0.,charsize=charsize,textcolors=djs_icolor('green'),pos=[.7,.014]
        djs_oplot, EJ[hyades_members,1], EJ[hyades_members,0],$
          color='yellow',psym=3
        legend, ['Hyades'], box=0.,charsize=charsize,textcolors=djs_icolor('yellow'),pos=[.7,.011]
        djs_oplot, EJ[hercules_members,1], EJ[hercules_members,0],$
          color='cyan',psym=3
        legend, ['Hercules'], box=0.,charsize=charsize,textcolors=djs_icolor('cyan'),pos=[.7,.008]
    ENDIF ELSE BEGIN
        djs_oplot, EJ[ngc1901_members,1], EJ[ngc1901_members,0],$
...
ENDIF ELSE IF output EQ 3 THEN BEGIN
    ...
    djs_plot, EJ[hercules_members,1], EJ[hercules_members,0],$
      ...
      psym=3, yrange=[0.,0.05], xrange=[0.7000001,1.3],charsize=charsize, $
